...
add_action( 'after_setup_theme', 'shiword_mobile_setup' );
// Register sidebars by running shiword_mobile_widget_area_init() on the widgets_init hook
add_action( 'widgets_init', 'shiword_mobile_widget_area_init' );
...
if ( !function_exists( 'shiword_mobile_device_detect' ) ) {
	function shiword_mobile_device_detect() {
		...
		if ( ! shiword_get_opt( 'shiword_mobile_css' ) ) return false;
...
if ( !function_exists( 'shiword_mobile' ) ) {
	function shiword_mobile () {
		...
		if ( $shiword_is_mobile ) {
			...
			add_filter( 'widget_categories_args', 'shiword_mobile_widget_categories_filter', 90 );
			add_filter( 'widget_archives_args', 'shiword_mobile_widget_archives_filter', 90 );
			add_filter( 'widget_pages_args', 'shiword_mobile_widget_pages_filter', 90 );
			add_filter( 'sw_widget_pop_categories_args', 'shiword_mobile_widget_pop_categories_filter', 90 );
...
if ( !function_exists( 'shiword_mobile_widget_area_init' ) ) {
	function shiword_mobile_widget_area_init() {
		...
		register_sidebar( array(
			...
			'before_title' => '</div>' . shiword_mobile_seztitle( 'before' ),
			'after_title' => shiword_mobile_seztitle( 'after' ) . '<div class="widget-body">',
		) );
...
if ( !function_exists( 'shiword_mobile_stylesheet' ) ) {
	function shiword_mobile_stylesheet(){
...
if ( !function_exists( 'shiword_mobile_post_details' ) ) {
	function shiword_mobile_post_details( $auth, $date, $tags, $cats, $hiera = false, $av_size = 48, $featured = false ) {
		global $post;
		?>
				...
				<div class="tbm-author-bio vcard">
					<ul>
						...
						<li class="author-social">
							<?php if ( get_the_author_meta('twitter', $author) ) echo '<a target="_blank" class="url" title="' . sprintf( __('follow %s on Twitter', 'shiword'), $name ) . '" href="'.get_the_author_meta('twitter', $author).'"><img alt="twitter" class="avatar" width="24" height="24" src="' . get_template_directory_uri() . '/images/follow/twitter.png" /></a>'; ?>
							<?php if ( get_the_author_meta('facebook', $author) ) echo '<a target="_blank" class="url" title="' . sprintf( __('follow %s on Facebook', 'shiword'), $name ) . '" href="'.get_the_author_meta('facebook', $author).'"><img alt="facebook" class="avatar" width="24" height="24" src="' . get_template_directory_uri() . '/images/follow/facebook.png" /></a>'; ?>
						</li>
					</ul>
				</div>
			<?php } ?>
...
if ( !function_exists( 'shiword_mobile_setup' ) ) {
	function shiword_mobile_setup() {
...
function shiword_mobile_tag_cloud_filter($args = array()) {
   $args['smallest'] = 1;
   $args['largest'] = 1;
   $args['unit'] = 'em';
   return $args;
...
function shiword_mobile_widget_categories_filter($args = array()) {
   $args['hierarchical'] = 0;
   $args['show_count'] = 0;
   return $args;
}

function shiword_mobile_widget_archives_filter($args = array()) {
   $args['show_post_count'] = 0;
   return $args;
}

function shiword_mobile_widget_pages_filter($args = array()) {
   $args['depth'] = 1;
   return $args;
}

function shiword_mobile_widget_pop_categories_filter($args = array()) {
   $args['show_count'] = 0;
   return $args;
}
