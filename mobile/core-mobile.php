...
class Shiword_Mobile {
	...
	function device_detect() {
		...
		if ( ! $this->get_option( 'shiword_mobile_css' ) ) return false;
	...
	function indexes_navigation(){
		...
		echo apply_filters( 'shiword_mobile_filter_seztitle', sprintf( __( 'page %1$s of %2$s', 'shiword' ) , $paged, $wp_query->max_num_pages ) );
	...
	function page_hierarchy(){
		...
		if ( !empty($sub_pages) ) {
			...
			?>
				<?php echo apply_filters( 'shiword_mobile_filter_seztitle', __( 'Child pages', 'shiword' ) ); ?>
				<ul class="tbm-group">
					<?php 
		...
		if ( $parent_page ) {
			?>
				<?php echo apply_filters( 'shiword_mobile_filter_seztitle', __( 'Parent page', 'shiword' ) ); ?>
				<ul class="tbm-group">
	...
	function search_reminder() {
		...
		if ( is_archive() ) {
			...
			$text = sprintf( __( 'Search results for &#8220;%s&#8221;', 'shiword' ), '<span class="search-term">' . esc_html( get_search_query() ) . '</span>' );

		} elseif ( is_404() ) {

			$text = __( 'Error 404', 'shiword' );

		}

		echo apply_filters( 'shiword_mobile_filter_seztitle', $text );
	...
	function comment_form_defaults( $defaults ) {
		...
		$defaults['title_reply']		= apply_filters( 'shiword_mobile_filter_seztitle', __( 'Leave a comment', 'shiword' ) );
		$defaults['title_reply_to']		= apply_filters( 'shiword_mobile_filter_seztitle', __( 'Leave a Reply to %s', 'shiword' ) );
	...
	function body_classes( $classes ) {
		...
		$classes[] = $this->get_option( 'shiword_mobile_css_color' );
